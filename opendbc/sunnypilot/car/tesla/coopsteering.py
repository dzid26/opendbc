...
STEER_DESIRED_LIMITER_OVERRIDE_ACTIVE_COUNTER = 0.4 # second
...
class SteerRateLimiter:
  ...
  def reset(self, apply_angle: float) -> None:
    ...
    self._last = apply_angle

  def update(self, apply_angle: float, angle_delta_lim: float) -> float:
    angle_lim = rate_limit(apply_angle, self._last, -angle_delta_lim, angle_delta_lim)
...
class CoopSteeringCarController:
  ...
  def steer_desired_accel_limit_for_override(self, lat_active: bool, apply_angle: float, vEgo: float, steeringTorque: float) -> float:
    ...
    if self.override_active_counter < STEER_DESIRED_LIMITER_OVERRIDE_ACTIVE_COUNTER and vEgo < STEER_DESIRED_LIMITER_ALLOW_SPEED:
      max_angle_accel = STEER_DESIRED_LIMITER_ACCEL
    else:
